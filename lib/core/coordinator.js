...
export class CheckinCoordinator {
      ...
      for (const target of discovered) {
        ...
        if (existing) {
          ...
          account.targets.push({
            ...
            subscription: {
              enabled: true,
              preferredHour: this.config.scheduler.defaultHour,
            },
          })
        }
      }
  ...
  async setTargetEnabled(identity, ordinal, enabled) {
    return this.store.transaction(data => {
      ...
      target.subscription ??= { preferredHour: this.config.scheduler.defaultHour }
  ...
  collectJobs(user, attempts, { communityId, respectSchedule, now }) {
    const jobs = []
      ...
      for (const target of account.targets ?? []) {
        ...
        if (
          ...
          hourInTimezone(target.businessTimezone, now) !==
            Number(target.subscription.preferredHour)
        ) continue
        jobs.push({ account, target, businessDate: date })
      }
    }
    return jobs
  }
  ...
  async runOne(user, job) {
    ...
    await this.recordAttempt(user.identity, job, outcome, attemptNo)
    return {
      targetId: job.target.id,
      targetName: job.target.displayName,
      ...outcome,
  ...
  async recordAttempt(identity, job, outcome, attemptNo) {
  ...
  publicTarget(target) {
    return {
      ...
      preferredHour: target.subscription?.preferredHour,
    }
  }
}
