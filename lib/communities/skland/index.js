...
const ARKNIGHTS_ITEM_ICON_BASE =
...
function arknightsItemIcon(resource) {
  if (resource?.icon || resource?.iconUrl) {
    return resource.icon ?? resource.iconUrl
  }
  const type = String(resource?.type ?? "")
  if (!/^[A-Za-z0-9_]+$/.test(type)) return undefined
  return `${ARKNIGHTS_ITEM_ICON_BASE}/${type}.png`
}

function endfieldRewards(entries, resourceInfoMap = {}) {
  return (entries ?? []).flatMap(entry => {
    const id = typeof entry === "string"
      ? entry
      : entry?.id ?? entry?.awardId
    const award = resourceInfoMap?.[id]
    return award?.name
      ? [{
          name: award.name,
          count: entry?.count ?? award.count,
          icon: award.icon,
        }]
      : []
  })
...
export class SklandAdapter extends CommunityAdapter {
    ...
    if (payload.code === 0) {
      return {
        ...
        rewards: (payload.data?.awards ?? [])
          .filter(award => award.resource?.name)
          .map(award => ({
            name: award.resource.name,
            count: award.count,
            icon: arknightsItemIcon(award.resource),
          })),
      }
    }
  ...
  async endfieldCheckIn(auth, metadata) {
    ...
    if (payload.code === 0) {
      return {
        kind: "success",
        rewards: endfieldRewards(
          payload.data?.awardIds,
          payload.data?.resourceInfoMap,
        ),
      }
    }
    if (payload.code === 10001) {
      return await this.endfieldAlreadyDone(auth, metadata)
    }
    return this.classify(payload)
  }

  async endfieldAlreadyDone(auth, metadata) {
    try {
      const url = new URL(END_FIELD_SIGN_URL)
      url.searchParams.set("gameId", "3")
      url.searchParams.set("roleId", metadata.roleId)
      url.searchParams.set("serverId", metadata.serverId)
      const payload = await this.signedRequest(url.toString(), auth, {
        extraHeaders: {
          "sk-game-role": `3_${metadata.roleId}_${metadata.serverId}`,
        },
      })
      if (payload.code !== 0 || !payload.data?.hasToday) {
        return { kind: "already-done" }
      }
      const latestClaimed = [...(payload.data?.calendar ?? [])]
        .reverse()
        .find(item => item?.done && item?.awardId)
      return {
        kind: "already-done",
        rewards: endfieldRewards(
          latestClaimed ? [latestClaimed] : [],
          payload.data?.resourceInfoMap,
        ),
      }
    } catch (error) {
      globalThis.logger?.warn?.(
        `[A-game-checkin] 终末地今日奖励查询失败：${error.message}`,
      )
      return { kind: "already-done" }
    }
  }
  ...
  async authenticate(token) {
    ...
    const generatedCode = generated.code ?? generated.status
    if (Number(generatedCode) !== 0 || !generated.data?.cred) {
      throw new Error(generated.message || generated.msg || "无法生成森空岛凭证")
    }
    const value = { ...generated.data }
    if (!value.token) {
      const refreshed = await this.requestJson(`${BASE_URL}/auth/refresh`, {
        headers: { cred: value.cred },
      })
      const refreshCode = refreshed.code ?? refreshed.status
      if (Number(refreshCode) !== 0 || !refreshed.data?.token) {
        throw new Error(refreshed.message || refreshed.msg || "无法刷新森空岛签名凭证")
      }
      value.token = refreshed.data.token
    }
