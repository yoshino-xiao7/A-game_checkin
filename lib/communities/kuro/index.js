...
export class KuroAdapter extends CommunityAdapter {
  ...
  normalizeCredential(input) {
    if (typeof input === "object" && input) {
      ...
      if (source.userId && source.token) {
        return { userId: String(source.userId), token: String(source.token) }
      }
    }
  ...
  async checkIn(input, target) {
    ...
    try {
      ...
      if (info.data?.isSigIn) {
        return {
          kind: "already-done",
          reward: await this.todayReward(credential, metadata),
        }
      }
  ...
  async todayReward(credential, metadata) {
    try {
      ...
      const reward = (payload.data ?? []).find(item =>
        String(item.sigInDate ?? "").startsWith(today),
      )
      return reward
        ? { name: reward.goodsName, count: reward.goodsNum }
        : undefined
    } catch {
      return undefined
    }
  ...
  headers(credential, browser = false) {
    const deviceCode = fixedDeviceCode(credential.userId)
    if (browser) {
      return {
        source: "android",
        "user-agent":
  ...
  async post(path, credential, data, browser = false) {
    ...
    try {
      const response = await this.fetchImpl(`${BASE_URL}${path}`, {
        method: "POST",
        headers: this.headers(credential, browser),
