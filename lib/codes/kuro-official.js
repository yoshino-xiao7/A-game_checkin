...
export function extractTrustedKuroCodes(payloads) {
  ...
  for (const evidence of payloads.flatMap(evidenceFromDetail)) {
    ...
    const expiresAt = expiryFromText(evidence.text)
    const strongContext = contextual && Boolean(expiresAt)
    for (const code of codes) {
      ...
      item.strongContext ||= strongContext
  ...
  return [...byCode.values()]
    .filter(item => item.trusted || item.strongContext || item.users.size >= 2)
    .filter(item => !item.expiresAt || Date.parse(item.expiresAt) > now)
    .map(({ users, trusted, strongContext, ...item }) => item)
}
