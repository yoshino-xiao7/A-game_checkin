import { CN_CODE_GAMES, resolveCodeGame } from "./miyoushe-live.js"
...
function publicSubscription(user) {
  return Object.values(CN_CODE_GAMES).map(game => ({
    gameKey: game.key,
    gameName: game.name,
...
export class CodeSubscriptionService {
  ...
  async scan() {
    ...
    )].filter(gameKey => CN_CODE_GAMES[gameKey])
